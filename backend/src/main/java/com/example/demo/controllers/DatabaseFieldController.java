...
@RequestMapping(value="/databaseField")
public class DatabaseFieldController {
    ...
    @ResponseBody
    @RequestMapping(value="/getListDatabaseFields", method = RequestMethod.GET)
    public Optional<List<DatabaseField>> findAllField() throws UsernameNotFoundException {
        String username = JwtUtils.getUserNameFromRequest(request);
        // Get userid using its name.
        User user = userRepository.findByUsername(username).orElseThrow(() -> new UsernameNotFoundException("User Not Found with username: " + username));
        Long userid = user.getId();
        return databaseFieldService.findAll(userid);
    }
}