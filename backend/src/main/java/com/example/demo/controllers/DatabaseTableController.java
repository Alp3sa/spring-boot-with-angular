...
@RequestMapping(value="/databaseTable")
public class DatabaseTableController {
    ...
    @ResponseBody
    @RequestMapping(value="/getListDatabaseTables", method = RequestMethod.GET)
    public Optional<List<DatabaseTable>> findAllTables() throws UsernameNotFoundException {
        String username = JwtUtils.getUserNameFromRequest(request);
        // Get userid using its name.
        User user = userRepository.findByUsername(username).orElseThrow(() -> new UsernameNotFoundException("User Not Found with username: " + username));
        Long userid = user.getId();
        return databaseTableService.findAll(userid);
    }
}